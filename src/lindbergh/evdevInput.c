...
int (*mySelect)(int nfds, fd_set *restrict readfds, fd_set *restrict writefds, fd_set *restrict exceptfds,
...
static const int maxval[EV_MAX + 1] = {
    [0 ... EV_MAX] = -1, [EV_SYN] = SYN_MAX, [EV_KEY] = KEY_MAX, [EV_REL] = REL_MAX, [EV_ABS] = ABS_MAX, [EV_MSC] = MSC_MAX,
    [EV_SW] = SW_MAX,    [EV_LED] = LED_MAX, [EV_SND] = SND_MAX, [EV_REP] = REP_MAX, [EV_FF] = FF_MAX,   [EV_FF_STATUS] = FF_STATUS_MAX,
};
...
ControllerStatus loadEvdevControllers(Controllers *controllers)
{
    ...
    for (int i = 0; i < controllers->count; i++)
    {
        snprintf(controllers->controller[i].path, sizeof(controllers->controller[i].path), "%s/%s", "/dev/input", namelist[i]->d_name);
        free(namelist[i]);
        ...
        ioctl(controller, EVIOCGPHYS(sizeof(controllers->controller[i].physicalLocation)), controllers->controller[i].physicalLocation);
        ...
        if (test_bit(EV_ABS, bit[0]))
        {
            ...
            for (int code = 0; code < KEY_MAX; code++)
            {
                if (test_bit(code, bit[EV_ABS]))
                {
                    ...
                    ControllerInput *minControllerInput = &controllers->controller[i].inputs[controllers->controller[i].inputCount++];
                    ...
                    ControllerInput *maxControllerInput = &controllers->controller[i].inputs[controllers->controller[i].inputCount++];
    ...
    for (int i = 0; i < controllers->count - 1; i++)
    {
        ...
        {
            Controller tmp;
            if (strcmp(controllers->controller[j].physicalLocation, controllers->controller[j + 1].physicalLocation) > 0)
            {
...
void *controllerThread(void *_args)
{
    ...
    while (args->controllers->threadsRunning)
    {
        ...
        if (mySelect(fd + 1, &file_descriptor, NULL, NULL, &tv) < 1)
            continue;
        ...
        if (read(fd, &event, sizeof(event)) == sizeof(event))
        {
            switch (event.type)
            {
                ...
                case EV_KEY:
                {
                    ...
                    {
                        setAnalogue(args->controller->keyTriggers[event.code].channel, event.value == 0 ? 0 : 1 * (pow(2, jvsBits) - 1));
                        continue;
                    }
                ...
                case EV_ABS:
                {
                    double scaled = ((double)event.value - (double)args->controller->absMin[event.code]) /
                    ...
                    if (args->controller->absTriggers[event.code].shakeEnabled)
                    {
                        ...
                        if ((scaled > args->controller->absTriggers[event.code].shakePreviousScaled &&
                             scaled - args->controller->absTriggers[event.code].shakePreviousScaled > 0.1) ||
                            (scaled < args->controller->absTriggers[event.code].shakePreviousScaled &&
                             -scaled + args->controller->absTriggers[event.code].shakePreviousScaled > 0.1))
                        {
...
ControllerStatus startControllerThreads(Controllers *controllers)
{
    for (int i = 0; i < controllers->count; i++)
    {
        ...
        for (int j = 0; j < controllers->controller[i].inputCount; j++)
        {
            ...
            switch (controllers->controller[i].inputs[j].evType)
            {
                case EV_ABS:
                {
                    switch (controllers->controller[i].inputs[j].specialFunction)
                    {
                        case NO_SPECIAL_FUNCTION:
                        {
                            ...
                            controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].channel = input.channel;
                            strncpy(controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].name, input.name,
                                    SIZE);
                            controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].player = input.player;
                        ...
                        case ANALOGUE_TO_DIGITAL_MAX:
                        {
                            ...
                            controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].maxChannel = input.channel;
                            strncpy(controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].maxName, input.name,
                                    SIZE);
                            controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].maxPlayer = input.player;
                            controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].isNeg = negabs;
                            if (strstr(input.name, "ANALOGUE") == NULL)
                                controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].isAnalogue = 0;
                        }
                        ...
                        case ANALOGUE_TO_DIGITAL_MIN:
                        {
                            ...
                            controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].minChannel = input.channel;
                            strncpy(controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].minName, input.name,
                                    SIZE);
                            controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].minPlayer = input.player;
                            controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].isNeg = negabs;
                            if (strstr(input.name, "ANALOGUE") == NULL)
                                controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].isAnalogue = 0;
                        }
                        ...
                        case ANALOGUE_SHAKE:
                        {
                            ...
                            strncpy(controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].shakeName,
                                    input.name, SIZE);
                            controllers->controller[i].absTriggers[controllers->controller[i].inputs[j].evCode].shakePlayer = input.player;
                        }
                        break;
                ...
                case EV_KEY:
                {
                    ...
                    controllers->controller[i].keyTriggers[controllers->controller[i].inputs[j].evCode].channel = input.channel;
                    strncpy(controllers->controller[i].keyTriggers[controllers->controller[i].inputs[j].evCode].name, input.name, SIZE);
                    controllers->controller[i].keyTriggers[controllers->controller[i].inputs[j].evCode].player = input.player;
        ...
        memcpy(args->analogue_deadzone_start, config->arcadeInputs.analogue_deadzone_start,
               sizeof(config->arcadeInputs.analogue_deadzone_start));
...
{
    // Don't start EVDEV if SDL mode is enabled
    if (getConfig()->inputMode == 1)
    {
        return CONTROLLER_STATUS_SUCCESS;
    }
    mySelect = dlsym(RTLD_NEXT, "select");

    if (loadEvdevControllers(controllers) != CONTROLLER_STATUS_SUCCESS)
    {
        return CONTROLLER_STATUS_ERROR;
    }
